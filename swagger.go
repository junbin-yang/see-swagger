...
// Config stores seeSwagger configuration variables.
type Config struct {
...
// WrapHandler wraps `http.Handler` into `see.HandlerFunc`.
func WrapHandler(h *webdav.Handler, confs ...func(c *Config)) see.HandlerFunc {
...
// CustomWrapHandler wraps `http.Handler` into `see.HandlerFunc`
func CustomWrapHandler(config *Config, handler *webdav.Handler) see.HandlerFunc {
	var once sync.Once
...
func DisablingWrapHandler(h *webdav.Handler, envName string) see.HandlerFunc {
...
// if specified environment variable passed
func DisablingCustomWrapHandler(config *Config, h *webdav.Handler, envName string) see.HandlerFunc {
	eFlag := os.Getenv(envName)
	if eFlag != "" {
		return func(c *see.Context) {
			// Simulate behavior when route unspecified and
